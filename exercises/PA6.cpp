...
f) age of member pointed to by personal_ptr of the structure pointed to by planet_ptr. 
...
h) origin of member pointed to by personal_ptr of the structure pointed to by planet_ptr. 
                cout<<planet_ptr->personal_ptr->origin<<endl;

2. Now have your group  member check your answers. After that, let’s confirm  them by writing 
them in code. Define the above structure and initialize the members with suitable values. Then 
compile and see how many of your answers had syntax errors? Report the number of errors 
{in comments}. Also fix the code to ensure that it works properly. You can leave your old code 
in comments. 
 
3. Now let’s do a little more extra work by updating your code (and testing it) to do the following: 
a) Initialize all members of all 5 planets stored in all_records and print them neatly. 
b) Create a pointer that would point to dynamically allocated array of records of 5 planets 
(similar to all_records[5]). Again, initialize all of its members and print them neatly using 
...
// makes array (with same data) of 5 planets in all records
  for (int i = 0; i < 5; i ++) {
    ...
    strcpy(akashic_records_ptr[i].planet_name, "Something cool");
    strcpy(akashic_records_ptr[i].personal.origin, "not space");
